...
class WeakLensingMetrics(Metric):
    def __init__(self, num_redshift_bins, **kwargs):
        ...
        self.add_state(
            "total", default=torch.zeros(num_redshift_bins), dist_reduce_fx="sum"
        )
    ...
    def update(self, true_cat, est_cat, matching) -> None:
        ...
        zero_baseline_shear1_sq_err = (
            (true_shear1 - zero_baseline_pred_shear1) ** 2
        ).sum([0, 1])
        ellip_baseline_shear1_sq_err = (
            (true_shear1 - ellip_baseline_pred_shear1) ** 2
        ).sum([0, 1])
        shear2_sq_err = ((true_shear2 - pred_shear2) ** 2).sum([0, 1])
        zero_baseline_shear2_sq_err = (
            (true_shear2 - zero_baseline_pred_shear2) ** 2
        ).sum([0, 1])
        ellip_baseline_shear2_sq_err = (
            (true_shear2 - ellip_baseline_pred_shear2) ** 2
        ).sum([0, 1])
    ...
    def compute(self):
        ...
        zero_baseline_shear1_mse = (
            self.zero_baseline_shear1_sum_squared_err / self.total
        )
        ellip_baseline_shear1_mse = (
            self.ellip_baseline_shear1_sum_squared_err / self.total
        )
        shear2_mse = self.shear2_sum_squared_err / self.total
        zero_baseline_shear2_mse = (
            self.zero_baseline_shear2_sum_squared_err / self.total
        )
        ellip_baseline_shear2_mse = (
            self.ellip_baseline_shear2_sum_squared_err / self.total
        )
