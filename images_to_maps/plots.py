    ...
    def update(
        ...
        self.est_shear1.append(
            sample_with_mode_tile.get(
                "shear_1", torch.zeros_like(target_cat_cropped["shear_1"])
            )
        )
        ...
        self.est_shear2.append(
            sample_with_mode_tile.get(
                "shear_2", torch.zeros_like(target_cat_cropped["shear_2"])
            )
        )

        if "convergence" not in target_cat_cropped:
            self.true_convergence.append(
                torch.zeros_like(target_cat_cropped["shear_1"])
            )
...
def plot_lensing_scatterplots(
    ...
    for b in range(num_redshift_bins):
        ...
        ax[b, 0].scatter(
            true_shear1[..., b].flatten().cpu(),
            est_shear1[..., b].flatten().cpu(),
            alpha=0.2,
        )
        ...
        ax[b, 1].scatter(
            true_shear2[..., b].flatten().cpu(),
            est_shear2[..., b].flatten().cpu(),
            alpha=0.2,
        )
