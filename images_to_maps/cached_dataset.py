...
warnings.filterwarnings(
    "ignore", ".*Total length of .* across ranks is zero.*", UserWarning
)
...
class ChunkingDataset(Dataset):
    ...
    def __init__(self, file_paths, shuffle=False, transform=None, seed=0) -> None:
        ...
        self.accumulated_file_sizes = torch.zeros(
            len(self.file_paths), dtype=torch.int64
        )
    ...
    def __getitem__(self, index):
        ...
        converted_sub_index = (
            index - self.accumulated_file_sizes[converted_index]
        ).item()
    ...
    def get_chunked_indices(self):
        ...
        if self.shuffle:
            ...
            for start, end in zip(
                right_shift_list, accumulated_file_sizes_list, strict=True
            ):
...
class CachedSimulatedDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "predict":
            self.predict_dataset = self._get_dataset(
                self.file_paths, self.nontrain_transforms
            )
            return None
    ...
    def _load_file_paths_and_slices(self):
        ...
        self.file_paths = [
            os.path.join(str(self.cached_data_path), f) for f in file_names
        ]

        self.slices = self.parse_slices(
            self.splits, len(self.file_paths), self.splits_type
        )
    ...
    def parse_slices(self, splits: str, length: int, splits_type: str = "percent"):
        ...
        for i, data_split in enumerate(splits.split("/")):
            if splits_type == "percent":
                slices[i] = slice(
                    *(
                        self._percent_to_idx(val, length)
                        for val in data_split.split(":")
                    )
                )
            else:  # count
                slices[i] = slice(
                    *(self._count_to_idx(val) for val in data_split.split(":"))
                )
        return slices
    ...
    def _get_dataset(self, sub_file_paths, defined_transforms, shuffle: bool = False):
        ...
        return ChunkingDataset(
            sub_file_paths, shuffle=shuffle, transform=transform, seed=self.seed
        )

    def _get_dataloader(self, my_dataset):
        distributed_is_used = dist.is_available() and dist.is_initialized()
        sampler_type = (
            DistributedChunkingSampler if distributed_is_used else ChunkingSampler
        )
        return DataLoader(
            my_dataset,
